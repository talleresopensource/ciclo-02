...
2. Agregar un brevisimo abstract del trabajo que resuma su area de trabajo
...
#Load the dataset using the YAML parameter which is a file path
...
```{r, agregate}
#Group dataset by ID to get individual mean and median movement measures
df %>% group_by(ID) %>%
  summarise(
  across(
...
#Group the dataset by ID, treatment and lights condition to get agregated measures for each individual on each condition while saving the result
df_new <- df %>% group_by(ID, Treatment, lights) %>%
  ...
  mutate(diferencia = media - mediana) %>% #Add a column with the difference between the mean and the media
  ungroup()
```
...
trat <- df %>% group_by(ID, Treatment) %>% summarise(media = )
#Adjust linear model
lm_trat <- lm(variable_dependiente ~ variable_independiente, data = trat)
...
t_light <- t.test(data = df_new)
#Tidy t test results to display as an html table
broom::tidy(t_light) %>% gt::gt()

```
Los resultados son similares a los del modelo lineal simple, el efecto de la luz sobre el movimiento es enorme con un valor **t** de `r round()`.
...
Vemos que el efecto de la luz es muy grande para el tratamiento 1 con un efecto similar (aunque menor, con un valor **t** de `r round()`) al de ambos tratamientos a la vez. 
...
```
Para el tratamiento 2 los resultados son tambien similares aunque mayores (con un valor **t** de `r round()`).
...
La presencia o ausencia de luces tiene un enorme impacto sobre el movimiento de los animales. 

Sin embargo los tratamientos experimentales empleados no tuvieron incidencia alguna ni interaccionaron con el efecto de las luces. 